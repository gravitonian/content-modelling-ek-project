...
:build_share
    ...
    docker compose -f "%COMPOSE_FILE_PATH%" rm -f ek-project-share
	call %MVN_EXEC% clean package -pl ek-project-share,ek-project-share-docker
EXIT /B 0
:build_acs
    docker compose -f "%COMPOSE_FILE_PATH%" kill ek-project-acs
    docker compose -f "%COMPOSE_FILE_PATH%" rm -f ek-project-acs
	call %MVN_EXEC% clean package -pl ek-project-integration-tests,ek-project-platform,ek-project-platform-docker
EXIT /B 0
:tail
...
EXIT /B 0
:prepare-test
    call %MVN_EXEC% verify -DskipTests=true -pl ek-project-platform,ek-project-integration-tests,ek-project-platform-docker
EXIT /B 0
:test
    call %MVN_EXEC% verify -pl ek-project-platform,ek-project-integration-tests
EXIT /B 0
:purge
